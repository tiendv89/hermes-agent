...
class TestE2EFullPipeline:
    ...
    def test_tasks_stage_approve_runs_full_pipeline_a_b_c_d(self, monkeypatch):
        ...
        # Step d: tasks created via API
        create_tasks_mock.assert_called_once_with(_WORKSPACE_ID, _FEATURE_ID, _TASKS_MD)
...
class TestE2EBackupCreateTasksSuccess:
    ...
    def test_success_passes_tasks_md_content(self, monkeypatch):
        _, create_mock = self._run(monkeypatch)
        tasks_md_arg = create_mock.call_args[0][2]
        assert "T1" in tasks_md_arg
        assert "T2" in tasks_md_arg
